...
*
*  Modifications copyright (c) 2016 Pawel Kubik
*
                  ...
                  const chr::high_resolution_clock::time_point deadline)
{
    ...
    for (;;) {
        ...
        if (ret == -1) {
            ...
            const std::string msg = "Error in poll: " + getErrorMessage();
            LOGE(msg);
...
void setFDFlag(const int fd, const int getOp, const int setOp, const int flag, const bool set)
{
    ...
    if (ret == -1) {
        std::string msg = "fcntl(): Failed to get FD flags: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
    ...
    if (ret == -1) {
        std::string msg = "fcntl(): Failed to set FD flag: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
}
...
int open(const std::string &path, int flags, mode_t mode)
{
    ...
    for (;;) {
        ...
        if (-1 == fd) {
            ...
            const std::string msg = "open() failed: " + getErrorMessage();
            LOGE(msg);
            throw UtilsException(msg);
        }
        break;
...
void shutdown(int fd)
{
    ...
    if (-1 == ::shutdown(fd, SHUT_RDWR)) {
        std::string msg = "shutdown() failed: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
}
...
int ioctl(int fd, unsigned long request, void *argp)
{
    ...
    if (ret == -1) {
        const std::string msg = "ioctl() failed: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
    return ret;
...
int dup2(int oldFD, int newFD, bool closeOnExec)
{
    ...
    if (fd == -1) {
        const std::string msg = "dup3() failed: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
    return fd;
...
void write(int fd, const void* bufferPtr, const size_t size, int timeoutMS)
{
    ...
    for (;;) {
        ...
        if (n >= 0) {
            ...
            const std::string msg = "Error during writing: " + getErrorMessage();
            LOGE(msg);
            throw UtilsException(msg);
        }
...
void read(int fd, void* bufferPtr, const size_t size, int timeoutMS)
{
    ...
    for (;;) {
        ...
        if (n >= 0) {
            ...
            const std::string msg = "Error during reading: " + getErrorMessage();
            LOGE(msg);
            throw UtilsException(msg);
        }
...
unsigned int getMaxFDNumber()
{
    ...
    if (-1 ==  getrlimit(RLIMIT_NOFILE, &rlim)) {
        const std::string msg = "Error during getrlimit: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
...
void setMaxFDNumber(unsigned int limit)
{
    ...
    if (-1 ==  setrlimit(RLIMIT_NOFILE, &rlim)) {
        const std::string msg = "Error during setrlimit: " + getErrorMessage();
        LOGE(msg);
        throw UtilsException(msg);
    }
}
    ...
    for(;;) {
        ...
        if (ret < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
                ...
                throw UtilsException("Error during recvmsg: " + getErrorMessage());
            }
        } else if (ret == 0) {
    ...
    for(;;) {
        ...
        if (ret < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
                ...
                throw UtilsException("Error during sendmsg: " + getErrorMessage());
            }
        } else if (ret == 0) {
            // Retry the sending
        } else {
